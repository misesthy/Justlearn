                                ...
                                @forelse($tickets as $ticket)
                                    <tr class="text-gray-700">
                                        ...
                                        <td class="px-4 py-3 text-sm">
                                            ...
                                            <span style="color: {{ $ticket->status->name == 'Closed' ? '#f00' : ($ticket->status->name == 'Open' ? '#006400' : '')  }}; font-weight:{{ $ticket->status->name == 'Closed' ? '700' : ($ticket->status->name == 'Open' ? '700' : '')  }}">{{  $ticket->status->name }}</span>
                                        </td>
                                        <td class="px-4 py-3 text-sm">
                                            {{-- {{  $ticket->priority()->first() ? $ticket->priority()->first()->name:'' }} --}}
                                            <span class="rounded-full bg-gray-50 px-2 py-2"
                                                style="background-color: {{ $ticket->priority()->first()->color }}; color:white">
                                                {{ $ticket->priority()->first() ? $ticket->priority()->first()->name : '' }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-3 text-sm">
                                        ...
                                        <td class="px-4 py-3 space-x-2">
                                            @role('agent')
                                                ...
                                                <a class="rounded-lg border-2 border-transparent bg-purple-600 px-4 py-2 text-center text-sm font-medium leading-5 text-white transition-colors duration-150 hover:bg-purple-700 focus:outline-none focus:ring active:bg-purple-600"
                                                    href="{{ route('feedbacks.create', $ticket) }}">
                                                    {{ __('Answer') }}
                                                </a>
                                            @endrole
                                            @role('admin')
