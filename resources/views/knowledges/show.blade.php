...
<h1>Détails de la connaissance</h1>

<p>Nom : {{ $knowledge->name }}</p>
<p>Module : {{ $knowledge->module->name }}</p>
<!-- Affichez les autres détails de la connaissance -->

<a href="{{ route('connaissance.edit', $knowledge->id) }}">Modifier</a>

<form action="{{ route('connaissance.destroy', $knowledge->id) }}" method="POST">
    @csrf
    @method('DELETE')
